defmodule ExPass.Structs.PassFields.HeaderFields do
  @moduledoc """
  Represents fields displayed in the header area of a pass.

  Header fields appear at the top of the pass and are typically used to display the most
  important identifying information, such as gate numbers for boarding passes or seat
  numbers for event tickets. These fields are prominently displayed and easily visible.

  HeaderFields inherits all properties from `ExPass.Structs.FieldContent` without additional fields.

  For more details, see the [Apple Developer Documentation](https://developer.apple.com/documentation/walletpasses/pass/headerfields).
  ...
  @doc """
  Creates a new HeaderFields struct.

  This function initializes a new HeaderFields struct with the given attributes.
  It validates all inherited fields from FieldContent.

  ## Parameters

    * `attrs` - A map of attributes for the HeaderFields struct. Defaults to an empty map.

  ## Returns

    * A new `%HeaderFields{}` struct.
  ...
  ## Examples
      ...
      iex> HeaderFields.new(%{key: "gate", value: "A12"})
      %HeaderFields{key: "gate", value: "A12"}

      iex> HeaderFields.new(%{key: "seat", value: "14A", label: "Seat"})
      %HeaderFields{key: "seat", value: "14A", label: "Seat"}

      iex> HeaderFields.new(%{key: "terminal", value: "2", label: "Terminal", text_alignment: "PKTextAlignmentCenter"})
      %HeaderFields{key: "terminal", value: "2", label: "Terminal", text_alignment: "PKTextAlignmentCenter"}
  """
